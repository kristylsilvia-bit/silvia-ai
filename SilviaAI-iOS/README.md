...
## 3. Enable Firebase and Google accounts

The iOS app supports Firebase email/password accounts and Google sign-in using the same Firebase web API key style as the website.

1. In Firebase Console, open **Authentication > Sign-in method**.
2. Enable **Email/Password** and **Google**.
3. Open **Project settings > Your apps > Web app** and copy the web API key. The web app's `appId` is not the Google sign-in client ID.
4. In Xcode, open `SilviaAI > Resources > Config.plist`.
5. Replace `PASTE_YOUR_FIREBASE_WEB_API_KEY_HERE` with that Firebase web API key.
6. In Google Cloud Console, create an OAuth client for iOS using the app bundle identifier `com.silviaai.app`, or your custom bundle identifier if you changed it.
7. Put that iOS OAuth **Client ID** under `GOOGLE_CLIENT_ID` in `Config.plist`. It looks like `1234567890-abc.apps.googleusercontent.com`.
8. Optional: put the matching **iOS URL scheme** / reversed client ID under `GOOGLE_REDIRECT_SCHEME`. It looks like `com.googleusercontent.apps.1234567890-abc`. If you leave the placeholder, the app derives it from `GOOGLE_CLIENT_ID`.

Without these keys, the chat can still run in guest mode, but account actions show a setup message.

## 4. Run on the iPhone simulator
...
### Firebase sign-in setup message

Open `Resources/Config.plist` and replace the placeholder value under `FIREBASE_API_KEY`.

### Google sign-in setup message

Open `Resources/Config.plist` and replace `GOOGLE_CLIENT_ID` with your Google OAuth iOS client ID. Make sure Google is enabled in Firebase Authentication. If Google shows an iOS URL scheme or reversed client ID, paste that under `GOOGLE_REDIRECT_SCHEME`.
...
- Firebase email/password sign-in, Google sign-in, account creation, password reset, sign-out, guest mode, and per-account local chat history.
- Settings for model selection, personality/system prompt, voice input, spoken replies, and clearing saved chats.
- Message copying, response regeneration, clear chat, animated typing indicator, haptics, keyboard-safe composer, image/file attachments, and long-message support.
- Markdown-style inline formatting and readable code block rendering.
- Generated image display with tap-to-preview, share, and save-to-Photos actions when the image model returns inline image data.
